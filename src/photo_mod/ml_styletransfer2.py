...
class VGGc(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer_num, layer in enumerate(self.model):
            ...
            if str(layer_num) in self.req_features[0:4]:
                features.append(x)
...
class VGGl(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer_num, layer in enumerate(self.model):
            ...
            # appending the activation of the selected
            #  layers and return the feature array
            if str(layer_num) in self.req_features[3:]:
                features.append(x)
...
modelc = VGGc().to(device).eval()
modell = VGGl().to(device).eval()
modelg = VGG().to(device).eval()
...
def image_trainer(label: int) -> Image:
    ...
    style_image = image_loader(DATAPATH / "picasso.jpg")
    ...
    for e in range(100):
        ...
        gen_features = modelc(generated_image)
        orig_feautures = model(original_image)
        style_features = modell(style_image)
